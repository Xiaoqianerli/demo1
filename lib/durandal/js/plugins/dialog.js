...
define(['durandal/system', 'durandal/app', 'durandal/composition', 'durandal/activator', 'durandal/viewEngine', 'jquery', 'knockout'], function (system, app, composition, activator, viewEngine, $, ko) {
    ...
    MessageBox.defaultSettings = { buttonClass: "btn", primaryButtonClass: "btn-primary autofocus", secondaryButtonClass: "btn-default", "class": "modal-content messageBox", style: null };
        ...
        compositionComplete: function (child, parent, context) {
            ...
            var setDialogPosition = function (childView, objDialog) {
                ...
                setTimeout(function () {
                    ...
                    $(objDialog.host).css('width', $childView.width() + 6 + 'px');
                    $(objDialog.host).css('height', $childView.height() + 6 + 'px');
                    $(objDialog.host).css('left', ($(window).width() - $childView.width()) / 2 + 'px');
                    $(objDialog.host).css('top', ($(window).height() - $childView.height()) / 2 + 'px');
                    //小白菜 add
